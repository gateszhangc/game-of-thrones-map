...
This document explains how the project replicates the `https://thegameofthronesmap.com` homepage and validates visual parity. Follow these steps whenever you need to resync the clone or rerun the automated checks.
...
1. Fetch the live HTML from the repository root:
   ```powershell
   (Invoke-WebRequest -Uri https://thegameofthronesmap.com -UseBasicParsing).Content | Out-File thegameofthronesmap_home.html -Encoding utf8
   ```
2. Extract the `<head>` and `<body>` fragments into the Next.js data directory:
   ```bash
   node scripts/extractHtml.js
   ```
3. Mirror first-party static assets (CSS, images, Cloudflare helpers) into `public/`:
   ```bash
   node scripts/mirrorAssets.js
   ```
   > If the upstream site omits certain assets, you can populate simple fallbacks with:
   > ```bash
   > node scripts/generatePlaceholders.js
   > ```
   > This seeds parchment/map textures and a simple hero background so the clone
   > keeps its styling even if assets are missing upstream.
4. Regenerate the static snapshot used for baseline comparisons:
   ```bash
   node scripts/updateOriginalHtml.js
   ```
   This writes the combined markup to `public/original.html`.
5. Refresh SEO assets (robots.txt, sitemap.xml):
   ```bash
